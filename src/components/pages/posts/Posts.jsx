...
function Posts() {
  ...
  const fetchData = useCallback(
    async (pageNumber, publishState) => {
      await dispatch(getPosts({ pageSize, pageNumber, state: publishState }));
    },
    [dispatch]
  );
  useEffect(() => {
  ...
  let tagsEX = JSON.parse(localStorage.getItem("tags"));
  const columns = [
    {
      title: "Title",
      dataIndex: "title",
      key: "title",
      width: "20%",
      render: (text, record) => <div>{text.substring(0, 30)}</div>,
    },
    {
      title: "Slug",
      dataIndex: "slug",
    ...
    },
    {
      ...
      render: (tags) => (
        <>
          {tags.map((tag) => {
            let data = tagsEX.find((tagData) => tagData._id === tag);
            return (
